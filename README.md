...
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  ...
  `actions` are the functions that are done to the state. `reducers` describe what each action does to the state. `store` is the immutable data in your program.

- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

  Application state is the state for the entire application, it is global. Component state would be used for a form because it does not need to leak outside of that single component.

- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?

  `redux-thunk` allows us to use async functions as middleware in our flow. 
